...
const models = {
  ...
  */
  ...
  proccessSwaps(swaps, tokenInfo, callback) {
    ...
    const addressFrom = BnbApiClient.crypto.getAddressFromPrivateKey(key.private_key, config.prefix);
  ...
  processSwap(swap, tokenInfo, key, seq, callback) {
    ...
    bnb.transfer(key.private_key, swap.bnb_address, (amount_n - fee_n).toFixed(2), tokenInfo.unique_symbol, 'DOS Swap', seq, (err, swapResult) => {
      if(err) {
        console.log(err)
